...
CONTAINER_NAME=$(jq -r '.containerName' /opt/stc/docker-config.json)
echo "[stc] mode=$(jq -r '.mode' /opt/stc/config.json)"
...
control_container() {
  ...
  "idle")
    ...
    echo "[stc] idle=$idle_time, idleThreshold=$idle_threshold"
